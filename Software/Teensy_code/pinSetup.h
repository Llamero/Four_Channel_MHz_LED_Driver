...
class pinSetup
{
  public:
    ...
    const static int ANALOG_SELECT = 7; //Switches between internal and external analog input
    const static int ALARM[2]; //Audible alarm
    const static int TOGGLE = 8; //Toggle switch input
    const static uint16_t DEBOUNCE = 200; //ms to wait for switch to stop bouncing
    
    const static int RESISTOR_TEMP = 41; //NTC thermistor monitoring current sense resistor temp
    ...
    const static int SCK = 13; //SPI Clock signal
    const static int MOSI = 11; //SPI master out signal
    const static int MISO = 12; //SPI master in signal
    const static int CS = 10;  //SPI component select
    const static int OUTPUTS[]; //5V output pins for external triggering/PWM
    const static int POWER_3v3[]; //Pins the provide additional 3v3 power
    ...
    const static int POT = 38; //Input voltage from potentiometer
    const static int INPUTS[6]; //6-channel analog/digital inputs

    const static int LED_FREQ = 14648; //LED driver PWM freq - default to 2^n multiple to have optimal dynamic range while staying outside the auditory range - https://www.pjrc.com/teensy/td_pulse.html
