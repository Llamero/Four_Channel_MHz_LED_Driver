...
void updateIntensity(){
  ...
  if(conf.c.led_active[current_status.s.led_channel]){ //Check if the channel is active
    ...
    if(external_analog){
      ...
      if(!(sync.s.mode == 2 && !current_status.s.mode)){ //If not in confocal mode, do apply PWM to interline pin
