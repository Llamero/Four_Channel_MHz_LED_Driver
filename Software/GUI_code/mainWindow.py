...
class Ui(QtWidgets.QMainWindow):
            ...
            with open(str(path[0]), 'r') as stream:
                ...
                if csv_headers is not None and row_count > 1 and len(csv_headers) >= len(widget_headers):
                    for column, data in enumerate(widget_headers):
                        if csv_headers[column] != data and column < 4:
                            self.message_box.setText("Error: CSV header \"" + data + "\" does not match table header \"" + widget_headers[column] + "\". Import aborted.")
                ...
                for row, row_data in enumerate(reader):
                    if len(widget_headers) > len(row_data):
                        self.message_box.setText("Error: Row #" + str(row+1) + " has only " + str(len(row_data)) + " cells. At least " + str(len(widget_headers)) + " cells are required.  Import aborted.")
                        self.message_box.exec()
                        return
                    for column, data in enumerate(row_data):
                        try:
                            ...
                            self.message_box.setText("Error: \"" + str(data) + "\" at row #" + str(row + 1) + " is not a number. Import aborted.")
                            self.message_box.exec()
                            return
                        if column == 0:
                            if data not in [0,1,2,3,4]:
                                self.message_box.setText("Error: \"" + str(data) + "\" at row #" + str(row+1) + " is not a valid LED integer (0-4). Import aborted.")
                                self.message_box.exec()
                                return
                        elif column in [1,2]:
                            if data < 0 or data > 100 or data is None:
                                self.message_box.setText("Error: \"" + str(data) + "\" at row #" + str(row+1) + " is not a valid percentage (0-100). Import aborted.")
                                self.message_box.exec()
                                return
                        elif column == 3:
                            if (data < 10e-6 and data != 0) or data > 3600 or data is None:
                                self.message_box.setText("Error: \"" + str(data) + "\" at row #" + str(row+1) + " is not a valid duration (0 or 0.00001-3600). Import aborted.")
                                self.message_box.exec()
                                return
    ...
    def saveSequence(self, widget):
        path = QtGui.QFileDialog.getSaveFileName(
                self, 'Save File', '', 'CSV(*.csv)')
        if not path.isEmpty():
            with open(str(path[0]), 'wb') as stream:
