...
import re
import serial
from serial.threaded import ReaderThread, Packetizer, FramedPacket
...
class usbSerial():
    def __init__(self):
        ...
        self.serial_thread = None #Instance of serial thread
    ...
    def connectSerial(self, port):
        try:
            ...
            Packetizer.TERMINATOR = b"\0" #Set packetizer packet framing byte to NULL for COBS framing
            with ReaderThread(self.active_serial, Packetizer) as self.serial_thread:
                self.serial_thread.run() #Start thread that monitors for incoming serial data
            return True
    ...
    class cobsPacketProcessor(Packetizer):

        def handle_packet(self, packet):
            read_buffer = cobs.decode(packet)  # Decode packet